...
const scraper: Scraper = async (request, page) => {
  ...
  try {
    ...
    wcsPDPVideoConfig.videos.forEach(d => {
      //@ts-ignore
      videos.push(d.src)
    })
  ...
  const noStock = await page.$$eval('.out-of-stock', noStock =>
    noStock.map((p: any) => p.getAttribute('data-attribute-value').toUpperCase()),
  )
  ...
  const bullets = await page.$eval('.swatches-product-details-container', (e: any) =>
    Array.from(e.querySelectorAll('li')).map((i: any) => i.textContent),
  )
  const sections = (
    await page.$$eval(
    ...
    description_placement:
      section?.title == 'Details' ? DESCRIPTION_PLACEMENT.MAIN : DESCRIPTION_PLACEMENT.ADJACENT,
  }))
  ...
  for (const v of variants) {
    for (const i of v) {
      //Gather images
      await page.goto(
        `https://images.jansport.com/is/image/JanSport/${productCode}_${i.variationId}_set?req=set,json,UTF-8&labelkey=label&handler=s7sdkJSONResponse`,
      )
      let jsonp = await (await page.content())
        .replace(',"");</pre></body></html>', '')
        .replace(
          '<html><head></head><body><pre style="word-wrap: break-word; white-space: pre-wrap;">/*jsonp*/s7sdkJSONResponse(',
          '',
        )
      ...
      for (const iz in json) {
        ...
        if (ind) {
          ...
          imgset.push(
            // @ts-ignore
            `https://images.jansport.com/is/image/${ind}?$VFDP-VIEWER-ZOOMVIEW-HERO$&wid=1003&hei=1166&fmt=jpg`,
          )
        }
      }
      ...
      let sizeChartUrl = `http://widget.tangiblee.com/desktop/${productCode.toLowerCase()}-${
        i.variationId
      }?domain=www.jansport.com`

      const variant = new Product(productCode + '-' + i.variationId, title, url)

      await page.goto(sizeChartUrl)
      let sc = await page.content()
      if (
        !(sc.includes('Oh no, 500') || sc.includes('We have no information about this product'))
      ) {
